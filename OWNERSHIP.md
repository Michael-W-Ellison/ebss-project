...
| Subject | Owner | Guard |
|---|---|---|
| **Resources** | `world::ResourceType` - what kinds exist, what bears when (`is_it_bearing`), what is grown (`is_it_grown`), **how fast it comes back (`how_fast_it_comes_back`, which `is_renewable` and so `remove_depleted_resources` now ask rather than answering for themselves)**, what is food (`is_it_food`, which `is_edible` and `raw_scent_strength` now both ask rather than answering for themselves) | `resources::all_resources_tests::every_resource_is_listed` - an exhaustive match that fails to compile if a variant is added and not listed in `all()`; `scent_tests::only_food_smells_of_food` and `nothing_anybody_eats_is_odourless` hold the scent table to the food list in both directions; `scent_tests::everything_a_person_eats_grows_back` and `anticipation_tests::a_patch_picked_bare_is_still_there_to_grow_back` hold the growth table to it and stop the world deleting anything that grows |
...
| **Decisions** | `analytics::wanting` - the drive ladder in `wanting/mod.rs`, one module per question below it. `Agent::how_long_before_this_asks` and `what_will_not_wait_for` are the only forward-looking part: how long before a need starts asking, and which need a job will not outlast | `anticipation_tests` - that the clock is read off each agent's own drive, that a need of the same standing does not interrupt, and that a one-turn job cannot be |
| **Keeping** | `world::Pit` - what a hole holds, how much slower things age in it (`how_much_slower_things_age`, which both the ageing pass and the decision to bury now read), and how long a given thing would keep in it (`how_long_this_would_keep`) | `larder_tests::a_pit_says_how_long_a_thing_will_keep_in_it` and `nothing_goes_in_the_ground_that_will_not_still_be_food_when_it_is_wanted` |
| **How long a thing has left** | `world::nutrition::FoodData::how_long_this_has_left` - its own clock, what preparing it bought, and how far it has already gone, in ticks. Asked by `Pit::how_long_this_would_keep` for what would survive a hole and by `Agent::find_best_food_to_eat` for what to eat first, so the pit and the plate cannot disagree about what is nearly gone | `keeping_it_tests::what_is_nearest_to_going_is_what_gets_eaten`, `nobody_eats_the_rot_first`, `larder_tests::a_pit_says_how_long_a_thing_will_keep_in_it` |
| **What nobody has to be taught** | `agents::Agent::what_anybody_is_born_knowing` - the one list of techniques every agent has from birth. Laying a thing out to dry is on it: it is what happens to anything left in the sun, not an invention. See ISSUES_FOUND.md #125 | `weather_tests::anybody_can_dry_a_thing_without_being_shown_first` and `asking_tests::a_meal_is_no_longer_worth_asking_about` |
| **How many animals a world holds** | `environment::fauna::AnimalManager::how_many_are_alive` - the living, not the records. Asked by every gate that decides whether there is room for another animal: breeding, initial spawning and immigration. `bury_the_dead` takes the fallen off the map so the two can never diverge again. `spawn_group` asks neither, on purpose: whether there is room is the caller's question and each caller means something different by it. See ISSUES_FOUND.md #127 | `ecology_tests::a_corpse_is_not_counted_among_the_living`, `the_dead_are_taken_off_the_map`, `corpses_do_not_fill_up_the_world`, and `a_world_with_nobody_in_it_does_not_empty_of_animals`, which is the end-to-end statement |
| **What a growing thing takes and gives back** | `world::soil::Soil::NUTRIENT_PER_UNIT_GROWN` for what a unit costs the ground, `RESIDUE_PER_UNIT_GROWN` for the half that stays where it grew - and the same number again for the half that falls when nobody picks it, which is what makes a patch nobody touches break even. `ResourceNode::regenerate_in_ground` draws and returns the first; `what_it_carries_falls_off` returns the second | `bearing_tests::what_nobody_picks_goes_back_into_the_ground` and `ground_nobody_harvests_is_no_poorer_a_year_later`; `ecology_tests::the_hedgerows_are_no_thinner_a_few_years_on` |
| **What a person can carry** | `agents::Inventory::effective_max_weight` for the limit; `Agent::how_much_too_much_i_am_carrying` for how far over it a load is, `what_i_would_set_down` for what goes and `how_much_of_this_i_would_set_down` for how much. `Simulation::what_nobody_can_carry_any_more` is the only thing that acts on it, once a turn, and it is an invariant rather than a decision: what cannot be carried is not carried, and it stays where the carrier stood. See ISSUES_FOUND.md #126 | `larder_tests::a_body_that_weakens_sets_down_what_it_can_no_longer_carry`, `what_is_set_down_is_still_there_to_be_picked_up`, `the_axe_and_the_supper_are_the_last_things_anybody_puts_down`, `only_as_much_goes_down_as_the_shortfall_wants`, `nobody_puts_anything_down_who_does_not_have_to` |
| **What somebody is in the middle of** | `agents::Errand`, kept across turns and put down rather than thrown away when something that will not wait takes the turn (`set_aside`, `stale`); `Simulation::stick_to_the_errand` is the only thing that starts, continues, suspends or ends one | `errand_tests` - that a walk is finished rather than re-decided, that going for a drink is not a change of mind, and that an errand left standing two days is let go |
| **Who a thing belongs to** | `world::belonging::Belongs` for the claim, which lives on the thing (`Pit::belongs`, `Building::belongs`); `Agent::may_i_use` for what that claim permits *this* asker, which cannot live on the thing because it depends who is asking; `Agent::is_this_my_kin` for what a household is, which is the `RelationshipMap` and not a new object | `belonging_tests` - an exhaustive match per variant of both enums; that the same hut reads differently to two people; that a friend is not kin; and, twice, that a claim orders and never refuses: `a_strangers_hole_is_still_an_answer_when_it_is_the_only_one` and `every_tile_that_was_cover_is_still_cover_to_somebody` |
| **Actions** | `environment::verbs` for what a verb needs and changes; `analytics::doing` for what it does. Where a precondition depends on something the table cannot see - what a hunt needs depends on the quarry - the table says `BareHands` and one function elsewhere owns it (`Simulation::could_bring_it_down`) | `anticipation_tests::no_verb_asks_for_one_rung_of_a_tool_ladder_by_name`, which fails if any verb names a tool by hand instead of asking for the trade |

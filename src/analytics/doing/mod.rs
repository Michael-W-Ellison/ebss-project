...
//! and three seeds run for six hundred turns give byte-identical worlds either
...
impl Simulation {
    ...
    pub(crate) fn execute_action(&mut self, action: &Action, agent_index: usize) -> ActionResult {
        ...
        match action {
            ...
            Action::Gather { resource_type } => self.gathering(resource_type, agent_index, &mut rng, turn_now),
            ...
            Action::Fight { animal_id, weapon } => self.fighting_a_beast(animal_id, weapon, agent_index, &mut rng, turn_now),
            Action::Tame { animal_id, food_type } => self.taming(animal_id, food_type, agent_index, turn_now),
            Action::CollectAnimalProduct { animal_id } => self.collecting_from_a_beast(animal_id, agent_index, turn_now),
            Action::HarvestPlant { plant_id } => self.harvesting_a_plant(plant_id, agent_index, &mut rng, turn_now),
            Action::SeekShelter => self.seeking_shelter(agent_index),
            Action::Socialize { target_agent_id } => self.socialising(target_agent_id, agent_index, &mut rng, turn_now),
            ...
            Action::MakeClothing { garment } => self.making_clothing(garment, agent_index, turn_now),
            Action::WearClothing { garment } => self.wearing_clothing(garment, agent_index),
            Action::TillSoil => self.tilling_soil(agent_index, turn_now),
            Action::TrySwapping { instead_of_making, instead_of, put_in } => self.trying_a_swap(instead_of_making, instead_of, put_in, agent_index, turn_now),
            Action::TakeFrom { from } => self.taking_from(from, agent_index, turn_now),
            ...
            Action::Examine { what } => self.examining(what, agent_index, &mut rng, turn_now),
            Action::Boil => self.boiling(agent_index),
            Action::Salt { what } => self.salting(what, agent_index),
            Action::Dry { what } => self.drying(what, agent_index, turn_now),
            Action::Excavate => self.excavating(agent_index, turn_now),
            Action::Cover { what } => self.covering(what, agent_index, turn_now),
            Action::PickUp { what } => self.picking_up(what, agent_index, turn_now),
            Action::PutDown { what } => self.putting_down(what, agent_index, turn_now),
            Action::AskAbout { who, what } => self.asking_about(who, what, agent_index),
            Action::Trade { with } => self.trading(with, agent_index, turn_now),
            Action::GoWithout { for_them } => self.going_without(for_them, agent_index),
            Action::GiveTo { to } => self.giving_to(to, agent_index, turn_now),
            Action::Work { verb, to } => self.working(verb, to, agent_index, &mut rng, turn_now),
            Action::Taste => self.tasting(agent_index, &mut rng),
            Action::TakeCutting => self.taking_a_cutting(agent_index, turn_now),
            Action::PlantCutting => self.planting_a_cutting(agent_index, turn_now),
            Action::TendField => self.tending_a_field(agent_index, turn_now),
            Action::SpreadMuck => self.spreading_muck(agent_index, turn_now),
            Action::Treat { who } => self.treating(agent_index, *who, turn_now),
            Action::Fish => self.fishing(agent_index, &mut rng, turn_now),
            Action::SetSnare => self.setting_a_snare(agent_index, turn_now),
            Action::CheckSnares => self.going_round_the_line(agent_index, turn_now),
            Action::Wait => self.waiting(agent_index, &mut rng),
            Action::Explore { direction } => self.exploring(direction, agent_index, turn_now),
        }
    }
}

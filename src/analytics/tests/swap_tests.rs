...
// --------------------------------------------------------------------------
// A man may set his axe down beside his own larder
// --------------------------------------------------------------------------
//
// #207 gave the kit one rule: it may not leave the pack for *any* reason, "not
// to make room for supper, and not on the chance that a basket is what a
// digging stick was missing". The second half is right and #206 proved it. The
// first half kills settlements: packs run 86% to 105% full all year carrying
// wood, a stoneknife, a basket, a handaxe, a digging stick, a shovel, a spear
// and a bow, with four items of food in them, while the larder holds ten
// thousand items and forty-two people die of hunger beside it - **33,486
// refusals of "No room in the pack for what is in the store"**. See #212.
//
// Destroying a tool in an experiment and setting one down at your own store are
// not the same act, and one rule was answering for both.

/// Out on the moor the kit is still not spare.
#[test]
fn a_man_does_not_shed_his_axe_in_the_middle_of_nowhere() {
    let mut simulation = a_person();
    empty_the_pack(&mut simulation);
    give(&mut simulation, "handaxe", 1);
    ...
    assert_eq!(
        simulation.population.agents[0].what_i_would_set_down(),
        None,
        "nothing here is spare, and a tool dropped on the moor is a tool lost"
    );
}

/// Beside a store it goes down, because it can be picked back up.
#[test]
fn but_he_will_set_it_down_beside_his_own_larder() {
    let mut simulation = a_person();
    empty_the_pack(&mut simulation);
    give(&mut simulation, "handaxe", 1);
    ...
    assert_eq!(
        simulation.population.agents[0]
            .what_i_would_set_down_beside_a_store()
            .as_deref(),
        Some("handaxe"),
        "a full pack beside a full larder is the one place the axe comes off"
    );
}

/// And everything that is not the kit goes first, however light it is.
#[test]
fn everything_else_goes_before_the_kit_does() {
    let mut simulation = a_person();
    empty_the_pack(&mut simulation);
    give(&mut simulation, "handaxe", 1);
    give(&mut simulation, "stone", 1);
    ...
    assert_eq!(
        simulation.population.agents[0]
            ...
            .as_deref(),
        Some("stone"),
        "the kit is last, not merely ranked below: the stone goes even though \
         an axe is the heavier thing"
    );
}

/// What carries never goes down, anywhere.
///
/// A basket is thirty of the very capacity being made room in, so setting it
/// down to make room is a loss taken twice - the room it held goes with it.
#[test]
fn what_everything_is_carried_in_never_goes_down() {
    let mut simulation = a_person();
    empty_the_pack(&mut simulation);
    give(&mut simulation, "basket", 1);
    ...
    assert_eq!(
        simulation.population.agents[0].what_i_would_set_down_beside_a_store(),
        None,
        "not even at a store: the pack is what the room is measured in"
    );
}

/// The decision and the executor answer alike.
///
/// The store branch gates on `could_i_take_another_handful_at_a_store` and the
/// executor sheds with `set_down_what_is_worth_less_than_food_at_a_store`. If
/// the gate were the narrower of the two a man would never be offered his own
/// larder - which is the fault `could_i_take_another_handful` was added to fix
/// the first time round, in its narrow form.
#[test]
fn the_gate_and_the_executor_ask_the_same_question() {
    let mut simulation = a_person();
    empty_the_pack(&mut simulation);
    give(&mut simulation, "handaxe", 1);

    let agent = &mut simulation.population.agents[0];
    agent.inventory.max_weight = agent.inventory.current_weight;
    let agent = &simulation.population.agents[0];

    assert!(
        !agent.could_i_take_another_handful(1.0),
        "on the moor a full pack of tools is a full pack"
    );
    assert!(
        agent.could_i_take_another_handful_at_a_store(1.0),
        "at a store it is not, and the gate must say so or the branch never runs"
    );
}

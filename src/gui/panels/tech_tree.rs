// src/gui/panels/tech_tree.rs
//! Visual technology tree panel showing progression through eras.

use egui::{Ui, Color32, Pos2, Vec2, Rect, Stroke, RichText, ScrollArea, Sense};
use crate::gui::state::{GuiState, TechTreeSnapshot, TechNodeData, TechStatus};
...
pub fn render_tech_tree(ui: &mut Ui, state: &mut GuiState) {
    // Header with close button - handle this before borrowing snapshot
    ...
    let tech_snapshot = match &state.tech_tree_snapshot {
    ...
    // Main content area with tech tree and details
    let available_height = ui.available_height();

    ui.horizontal(|ui| {
        // Tech tree visualization (left side)
        ...
        egui::Frame::none()
            ...
            .show(ui, |ui| {
                ...
                render_tech_tree_visual(ui, state, &tech_snapshot);
            });
        ...
        ui.vertical(|ui| {
            ...
            render_tech_details(ui, state, &tech_snapshot);
        });
    });
}

fn render_progress_summary(ui: &mut Ui, snapshot: &TechTreeSnapshot) {
    ui.horizontal(|ui| {
        // Current era
        ui.vertical(|ui| {
...
fn render_tech_tree_visual(ui: &mut Ui, state: &mut GuiState, snapshot: &TechTreeSnapshot) {
    ScrollArea::both()
        ...
        .show(ui, |ui| {
            ...
            // Position nodes within their era columns
            let mut era_y_offsets: Vec<f32> = vec![ERA_HEADER_HEIGHT + 20.0; ERAS.len()];

            // Sort nodes by era for consistent positioning
            ...
            for node in &snapshot.nodes {
                if let Some(&center) = node_positions.get(&node.id) {
                    ...
                    draw_tech_node(ui, &painter, node_rect, node, state);
                }
            }
            ...
            if response.clicked() {
                if let Some(pos) = response.interact_pointer_pos() {
                    ...
                    state.selected_tech = clicked_node;
                }
            }
        });
}
...
fn draw_tech_node(
    ...
    // Selection highlight
    let is_selected = state.selected_tech.as_ref() == Some(&node.id);
    ...
    // Draw tech name (truncated if needed)
    ...
    // Draw progress bar for in-progress techs
    ...
    // Draw agents count for discovered techs
...
fn render_tech_details(ui: &mut Ui, state: &mut GuiState, snapshot: &TechTreeSnapshot) {
    ui.heading("Details");
    ui.separator();

    let Some(selected_id) = &state.selected_tech else {
